...
def run():
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        read_from_bigquery_write_to_avro_products = (
                p
                | 'ReadingProductsDataFromBigQuery' >> bq.ReadFromBigQuery(
            query=transform.query('products'),
            use_standard_sql=True,
            gcs_location=transform.gcs_bq_temp_loc(),
            project=transform.gcloud_project()
        )
                | 'WriteToAVRO' >> avroio.WriteToAvro(
            file_path_prefix=transform.avro_file_path_prefix_for_table('products'),
            schema=transform.get_avro_schema('products'),
            codec='deflate',
            use_fastavro=True,
